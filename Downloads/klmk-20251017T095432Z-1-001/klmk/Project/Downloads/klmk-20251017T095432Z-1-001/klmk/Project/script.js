...
async function initializeApp() {
    ...
    try {
        ...
        const incrementResponse = await fetch(`${API_URL}/visitors/increment`, {
            method: 'POST',
            headers: {
        ...
        updateVisitorNumber(incrementData.visitorNumber);
    } catch (error) {
...
function updateVisitorNumber(number) {
...
function renderProjects(projects = AppState.projects) {
    ...
    projectsGrid.innerHTML = projects.map(project => `
        <div class="project-card" data-project-id="${project.id}" onclick="showProjectDetails(${project.id})">
            <div class="project-content">
                <h3 class="project-title">${project.title}</h3>
                <p class="participant-name">
                    ${project.students ? project.students.join(', ') : 'Participants to be added'}
                </p>
            </div>
            <div class="project-details">
                <button class="edit-button" onclick="editProject(${project.id}, event)">
                    <i class="fas fa-edit"></i> Edit
                </button>
                <div class="project-images">
                    ${project.images ? project.images.map(img => 
                        `<img src="${img}" alt="Project Image" onclick="showFullImage(this.src, event)">`
                    ).join('') : '<p>No images added yet</p>'}
                </div>
                <div class="supervisor-info">
                    <strong>Supervisor:</strong> ${project.supervisor || 'Not assigned'}
                </div>
                <div class="participant-info">
                    <strong>Team Members:</strong><br>
                    ${project.students ? project.students.map(student => 
                        `<span class="student-name">${student}</span>`
                    ).join(', ') : 'No participants added'}
                </div>
            </div>
        </div>
    `).join('');
...
function initializeMap() {
    const schoolLocation = { lat: 28.5776667, lng: 77.0718707 };
    ...
    const marker = new google.maps.Marker({
        ...
        map: map,
        title: 'Shiksha Bharati Global School'
    });
}
